...
# Also load the repository-root .env (one level above backend/) so shared keys
# placed there — e.g. GOOGLE_GEMINI_API — are picked up. Values already set
# (from the environment or backend/.env above) take precedence.
...
# "celery" (default) queues post-upload work to a Celery worker. Set "thread"
# on deployments that run only the web process (no worker) so resume
# extraction/parsing runs in a background thread instead of sitting unread in
# the Redis queue forever.
BACKGROUND_TASKS_MODE = os.getenv("BACKGROUND_TASKS_MODE", "celery")
...
# ---------------------------------------------------------------------------
# Gemini (primary generative LLM)
# ---------------------------------------------------------------------------
# When GEMINI_API_KEY is set, Gemini is used as the primary model for structured
# generation (resume parsing, interview questions), with the Ollama LLM_MODEL /
# LLM_FALLBACK_MODELS above kept as automatic fallback. Uses a cheap "flash"
# model by default; override with GEMINI_MODEL (e.g. gemini-2.5-flash-lite).
GEMINI_API_KEY = os.getenv("GOOGLE_GEMINI_API", "") or os.getenv("GEMINI_API_KEY", "")
...
# Disable "thinking" tokens by default — cheaper and avoids empty completions on
# structured-JSON tasks. Set >0 (or -1 for dynamic) to re-enable.
