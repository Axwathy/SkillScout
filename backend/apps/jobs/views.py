...
from apps.candidates.tasks import dispatch_resume_extraction
...
def attach_resume_to_application(application, file, uploaded_by=None) -> Resume:
    ...
    dispatch_resume_extraction(resume, file_bytes)
    ...
    def create(self, request, *args, **kwargs):
        ...
        if resume_file:
            try:
                ...
                # Resume storage (Supabase) being unreachable shouldn't fail the
                # whole application — the candidate's submission is still valid
                # without the file attached.
                logger.exception(
                    "Failed to attach resume for application %s (job %s)",
                    application.id,
                    self.job.id,
                )
                resume_warning = (
                    "Your application was submitted, but the resume file could not "
                    "be uploaded right now. Please try uploading it again later."
                )
        audit_action = (
            "application.created"
        ...
        # Deliberately minimal: this endpoint is unauthenticated, and echoing
        # the full ApplicationSerializer would leak the candidate's stored
        # profile (resumes, skills, parsed data, scores) to anyone who posts a
        # known email address.
        response_data = {
        ...
        if resume_warning:
            response_data["resume_warning"] = resume_warning
        return Response(response_data, status=self.get_success_status(serializer))
