...
from apps.ai_engine.llm import gemini_generate_text, is_gemini_model
...
def parse_resume_text(raw_text: str) -> ParseResult:
    ...
    try:
        ...
        # Never let a heuristic-fallback bug mask the per-model errors: surface
        # everything that went wrong so the stored validation_errors are useful.
        raise ValueError(
            "; ".join([*validation_errors, f"heuristic-fallback: {exc}"])
        ) from exc
    return ParseResult(
...
def llm_parse_models() -> list[str]:
    configured = []
    # Gemini is the primary model when a key is configured; the Ollama models
    # below stay as automatic fallback.
    if getattr(settings, "GEMINI_API_KEY", ""):
        configured.append(getattr(settings, "GEMINI_MODEL", "gemini-2.5-flash"))
    configured.append(getattr(settings, "LLM_MODEL", "gpt-oss:20b"))
    configured.extend(getattr(settings, "LLM_FALLBACK_MODELS", []))
    models = []
...
def request_ollama_parse(raw_text: str, model: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if is_gemini_model(model):
        raw_response, token_usage = gemini_generate_text(prompt, model, temperature=0)
        if not raw_response:
            raise ValueError(f"Model '{model}' returned an empty response.")
        return parse_json_object(raw_response), token_usage
...
def infer_education(sections: dict[str, list[str]]) -> list[Education]:
    ...
    while index < len(lines):
        ...
        # group(2) may be the literal "present" ("2022 – Present"); only use it
        # when it is an actual year, otherwise fall back to the start year.
        end_year = year_match.group(2)
        graduation_year = (
            int(end_year) if end_year and end_year.isdigit() else int(year_match.group(1))
        )
