...
def test_ollama_parse_tolerates_real_resume_dates_and_estimates_confidence(settings):
    settings.GEMINI_API_KEY = ""  # pin to the Ollama chain this test asserts
    settings.LLM_MODEL = "gpt-oss:20b"
    payload = {
        "personal_info": {
...
def test_heuristic_fallback_handles_present_in_education_dates(settings):
    """Regression: '2022 – Present' in an education section used to crash the
    whole parse task with `int('Present')` instead of falling back cleanly."""
    settings.LLM_MODEL = "gpt-oss:20b"
    ...
    resume_text = (
        ...
        "parthiv@example.com\n"
        "EDUCATION\n"
        "B.Tech Computer Science 2022 – Present\n"
    ...
    assert education, "education section should be extracted"
    assert education[0]["graduation_year"] == 2022
